...
def main():
    ...
    parser.add_argument('-model-prefix',  dest='modelprefix', type=str, help='prefix of custom model used for prediciton. If you do not have one, please run train_general.py to train a model.', required=False,default=None)
    parser.add_argument('-residue-types',  dest='residues', type=str, help='Residue types that to be predicted. For multiple residues, seperate each with \',\'',required=False,default="C,H,E,D")
    ...
    if modelprefix is None:
      ...
      try:
          ...
          print('cannot open '+ parameter+" ! check if the model exists. please run train_models.py to get the custom model first!\n")
      else:
           f= open(parameter, 'r')
           parameters=f.read()
           f.close()
